...
def verificar_carrito_activo(user_id: int):
    try:
        ...
        if not usuario["existe"]:
            return {
                "success": False,
                "message": f"❌ No se puede crear/obtener carrito porque el usuario {user_id} no existe"
            }
        ...
        if not carrito:
            ...
            query_insert = """
                ...
                VALUES (%s, NOW(), 'activo')
            """
            new_car_id = execute_query(query_insert, (user_id,), commit=True, return_id=True)
