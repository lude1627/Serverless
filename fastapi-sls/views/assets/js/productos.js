        ...
        if (result.success && result.data && result.data.length > 0) {
            result.data.forEach(prod => {
                ...
                tr.innerHTML = `
                    ...
                    <td class="text-end">
                        <button class="btn btn-sm btn-warning" onclick="abrirModalEditar(${prod.id})">✏ Editar</button>
...
async function cargarCategoriasSelect(selectId, selectedValue = null) {
    ...
    try {
        ...
        select.innerHTML = '<option value="" disabled selected>Cargando categorías...</option>';
        ...
        if (result.success && Array.isArray(result.data) && result.data.length > 0) {
            ...
            select.innerHTML = '<option value="" disabled>No hay categorías disponibles</option>';
        }
...
document.getElementById("formAgregarProducto").addEventListener("submit", async (e) => {
    ...
    try {
        ...
        if (result.success) {
            ...
            Swal.fire("⚠️ Error", result.message || "No se pudo agregar el producto", "error");
        }
    } catch (error) {
        console.error("Error agregando producto:", error);
        Swal.fire("❌ Error", "No se pudo conectar al servidor. Verifica que el backend esté ejecutándose.", "error");
    }
});
...
async function abrirModalEditar(id) {
    try {
        ...
        document.getElementById("editProductoIdVisible").value = producto.product_id;
        document.getElementById("editName").value = producto.product_name;
        document.getElementById("editDescription").value = producto.product_description;
...
document.getElementById("formEditarProducto").addEventListener("submit", async function (e) {
    ...
    const category_id = parseInt(document.getElementById("editCategory_id").value);

    if (!name || !description || isNaN(price) || isNaN(cant) || isNaN(category_id)) {
        Swal.fire("⚠️ Campos incompletos", "Por favor llena todos los campos correctamente", "warning");
        return;
    }

    const productoActualizado = { id, name, description, price, cant, category_id };
    ...
    try {
        ...
        await Swal.fire("✅ Actualizado", result.message || "El producto se actualizó correctamente", "success");
        ...
        if (result.isConfirmed) {
            try {
                ...
                if (result.success) {
                    ...
                    Swal.fire("⚠️ Error", result.message || "No se pudo eliminar", "error");
                }
